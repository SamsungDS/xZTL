...
static void test_zrocks_buffer_read(uint64_t slba, void *buf_read,
    ...
    read = 0;
    while (size) {
        ret = zrocks_read_metadata(slba, (char *) buf_read + read,                // NOLINT
...
static void test_zrocks_metadata_read_failed(void) {
    ...
    if (!buf_read)
        goto FREE;
    test_zrocks_metadata_fill_buffer(buf_read, size);
    int ret;
    ret = zrocks_read_metadata(slba, (char *) buf_read,         // NOLINT
            READ_MAX_SZ_PER_COMMAND * 100);
    CU_ASSERT(ret != 0);

    FREE: zrocks_free(buf_read);
}
