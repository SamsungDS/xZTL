...
int zrocks_write(void *buf, size_t size, int level, struct zrocks_map maps[],
    ...
    if (level < 0) {
        log_erra("zrocks_write: error level [%d]\n", level);
    ...
    for (i = 0; i < ucmd.pieces; i++) {
        ...
        struct ztl_pro_node *node =
            (struct ztl_pro_node *)(&node_grp->vnodes[maps[i].g.node_id]);
        xztl_atomic_int64_update(&node->nr_valid, node->nr_valid + ucmd.num[i]);
        if (node->optimal_write_sec_left == 0) {
...
int zrocks_trim(struct zrocks_map *map) {
    ...
    xztl_atomic_int64_update(&node->nr_valid, node->nr_valid - map->g.num);
    if (ZROCKS_DEBUG)
        log_infoa("zrocks_trim: node ID [%u] contain invalid [%u]\n", node->id,
                  node->nr_valid);
