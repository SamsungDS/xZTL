...
static int _ztl_io_get_read_rs(void) {
    ...
    for (id = 0; id < XZTL_READ_RS_NUM; id++) {
...
static int _ztl_io_write_rs_init(struct ztl_queue_pool *q) {
    ...
    q->tctx = xztl_ctx_media_init(XZTL_CTX_NVME_DEPTH);
    if (!q->tctx) {
...
static int _ztl_io_read_rs_init(struct ztl_read_rs *r) {
    ...
    r->tctx = xztl_ctx_media_init(XZTL_CTX_NVME_DEPTH);
    if (!r->tctx) {
        log_err("_ztl_thd_init: Thread resource (tctx) allocation error.");
        return XZTL_ZTL_IO_ERR;
    }
...
static void ztl_io_read_callback_mcmd(void *arg) {
    ...
    if (mcmd->status) {
        ...
        memcpy(ucmd->buf + mcmd->buf_off, (char *)mcmd->prp[0] + misalign,
               mcmd->cpsize);  // NOLINT
    }
...
static uint32_t ztl_io_write_ncmd_prov_based(struct app_pro_addr *prov) {
    ...
    for (zn_i = 0; zn_i < prov->naddr; zn_i++) {
        ncmd += prov->nsec[zn_i] / ZTL_IO_SEC_MCMD;
        if (prov->nsec[zn_i] % ZTL_IO_SEC_MCMD != 0)
            ncmd++;
    }
...
int ztl_io_read_ucmd(struct xztl_io_ucmd *ucmd, struct ztl_read_rs *r) {
    ...
    int      nlevel, ncmd, cmd_i, total_cmd, zone_i, submitted;
    int      ret = 0;
    ...
    /*printf("misalign=%lu sec_size=%lu sec_start=%lu nlevel=%d zindex=%lu
       zone_sec_off=%lu read_num=%lu\r\n", misalign, sec_size, sec_start,
       nlevel, zindex, zone_sec_off, read_num);*/
    ...
    while (sec_left) {
        ...
        // printf("ztl_io_read_ucmd : node_id %d, ZN: %d, start: 0x%lx, start:
        // %d, nsec: %d\n", node_id, mcmd->addr[0].g.sect / 24576,
        // mcmd->addr[0].g.sect, mcmd->addr[0].g.sect % 24576, mcmd->nsec[0]);
...
int ztl_io_write_ucmd(struct xztl_io_ucmd *ucmd) {
    ...
    get_xztl_core(&core);
    uint32_t nsec, nsec_zn, ncmd, submitted, zn_i;
    uint64_t boff;
    int      ret, ncmd_zn, num, left;
    int      i, cmd_i;
    ...
    left = ncmd;
reget:
    // printf("START: 0x%lx   %d   %d  %d\r\n", ucmd, ucmd->pieces, left, ncmd);
    ...
    /* Update used and left optimal write sectors */
    ...
    while (cmd_i < num) {
        for (zn_i = 0; zn_i < prov->naddr; zn_i++) {
            ...
            // printf("ztl_io_write_ucmd: node_id %d, ZN: %d, start: 0x%lx,
            // start: %d, nsec: %d\n", q->node->id, mcmd->addr[0].g.zone,
            // mcmd->addr[0].g.sect, mcmd->addr[0].g.sect % 24576,
            // mcmd->nsec[0]);
...
static void *ztl_io_write_th(void *arg) {
    ...
    NEXT:
        if (!STAILQ_EMPTY(&q->ucmd_head)) {
            ...
            // printf("ztl_io_write_th: 0x%lx   %d   %d  0x%lx\r\n", ucmd,
            // ucmd->size, ucmd->prov_type, q);
...
static void ztl_io_submit(struct xztl_io_ucmd *ucmd) {
    ...
    // printf("ztl_io_submit: 0x%lx   %d   %d  0x%lx\r\n", ucmd, ucmd->size,
    // ucmd->prov_type, q);
...
static void ztl_io_exit(void) {
    ...
    for (rn = 0; rn < XZTL_READ_RS_NUM; rn++)
